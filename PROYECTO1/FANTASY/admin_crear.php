...
$errores = [];
$datos = ['nombre' => '', 'posicion' => '', 'equipo' => '', 'precio' => '', 'puntos' => ''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        $stmt = $pdo->prepare("INSERT INTO jugadores (nombre, posicion, equipo, precio, puntos) VALUES (?, ?, ?, ?, ?)");
        if ($stmt->execute([$datos['nombre'], $datos['posicion'], $datos['equipo'], $datos['precio'], $datos['puntos']])) {
            header('Location: admin_dashboard.php?msg=Jugador creado correctamente');
            exit;
        } else {
            $errores[] = 'Error al crear el jugador';
        }
    }
}

$page_title = 'Crear Jugador - Fantasy Fútbol';
...
<main class="container">
  <div class="row justify-content-center">
    <div class="col-md-8">
      <div class="card">
        <div class="card-body">
          <h2 class="mb-4"><i class="bi bi-plus-circle"></i> Crear Nuevo Jugador</h2>
