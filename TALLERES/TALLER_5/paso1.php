...
$ciudades = ["Nueva York", "Tokio", "Londres", "París", "Sídney", "Río de Janeiro", "Moscú", "Berlín", "Ciudad del Cabo", "Toronto", "Sudan"];
...
// 8. Crear una función que cuente y retorne el número de ciudades que comienzan con una letra específica
function contarCiudadesPorInicial($ciudades, $letra) {
    $contador = 0;
    foreach ($ciudades as $ciudad) {
        if (strpos($ciudad, $letra) === 0) {
            $contador++;
        }
    }
    return $contador;
...
// Tu código aquí
echo "\nNúmero de ciudades que empiezan con 'S': " . contarCiudadesPorInicial($ciudades, 'S') . "\n";
echo "Número de ciudades que empiezan con 'M': " . contarCiudadesPorInicial($ciudades, 'M') . "\n";
